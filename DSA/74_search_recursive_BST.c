...
struct Node* search(struct Node* root,int key)
{
    ...
    if(root->data==key)
        ...
        search(root->left,key);//we are using recursion to find the left subtree since current node's data is greater than the key
    
    else    
        search(root->right,key);//we are using recursion to find the right subtree since current node's data is lesser than the key
}
