...
void show(struct myArray* a)//function to show myArray's elments,*a is done to access all values(total_size,used_size) inside structure by derefercing  whereas & is for referencing
{
    ...
    {
        printf("%d\n",(a->ptr)[i]);//(a->ptr)[i] is used to point to the ith index of myArray using pointer
    }
}
...
int main()
{
    struct myArray marks;//creating an instance of the ADT myArray named marks.as we do this compiler makes a struture and memory is allocated for the data members inside structure like total_size,used_size and pointer*ptr
    createArray(&marks,20,3);//we passed address of marks to the pointer and we pass the total size of myArray and used size of myArray 
    setVal(&marks);
    printf("The values of myArray : \n");
    show(&marks);//we get garbage value if we don't use setVal function to set the values
    return 0;
